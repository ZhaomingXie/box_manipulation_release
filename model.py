...
class ActorCriticNet(nn.Module):
    ...
    def get_value(self, inputs, device="cpu"):
        x = F.elu(self.v_fcs[0](inputs))
        for i in range(len(self.hidden_layer)-1):
            x = F.elu(self.v_fcs[i+1](x))
        v = self.v(x)
        return v
...
class Discriminator(nn.Module):
    def __init__(self, num_inputs, hidden_layer=[64, 64]):
        ...
        self.mean_state = torch.zeros(num_inputs//2).to(device)
        self.std_state = torch.zeros(num_inputs//2).to(device)
    ...
    def compute_disc(self, state, next_state):
        disc_input = torch.cat([(state - self.mean_state)/self.std_state, (next_state - self.mean_state)/self.std_state], dim=-1)
    ...
    def set_normalizer(self, mean, std):
        ...
        self.mean_state[:] = torch.from_numpy(mean).to(device)
        self.std_state[:] = torch.from_numpy(std).to(device)
